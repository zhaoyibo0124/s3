...
import {
    intContentRuleDeleteApi,
    intContentRuleDisableApi,
    intContentRuleRecoveryApi,
    intContentRuleQueryListApi,
    modelListApi,
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    }>{record.catalogueName}</a-tooltip>,
                    ellipsis: true,
                    width: 200,
                },
                {
                    ...
                    customRender: (text, record) => <span>{this.queryJsonBasicList.yesNoType.filter(item => item.code == record.isAccurate).map(item =>
                        item.name)}</span>
                },
                {
                    title: '是否标题',
                ...
                },
                {
                    title: '是否相同',
                    dataIndex: 'isSame',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.yesNoType.filter(item => item.code == record.isSame).map(item =>
                        item.name)}</span>
                },
                {
                    title: '目录排序',
                    dataIndex: 'catalogueSort',
                },
                {
                    title: '操作',
                    dataIndex: 'action',
    ...
    methods: {
        ...
        queryListData(page, pageSize) {
            ...
            intContentRuleQueryListApi(data).then(res => {
                // 全局loading隐藏
        ...
        deleteBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    intContentRuleDeleteApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
        ...
        disableBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    intContentRuleDisableApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    intContentRuleRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

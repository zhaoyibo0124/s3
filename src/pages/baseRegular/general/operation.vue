		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="范本" prop="templateId" v-if="modelOptions.length > 0">
				<a-select :show-search="true" option-filter-prop="children" v-model="form.templateId" placeholder="请选择"
					:disabled="title == '查看'">
					<a-select-option v-for="item, index in modelOptions" :key="index" :value="item.id">{{ item.templateName
						}}</a-select-option>
				</a-select>
			</a-form-model-item>
			<a-form-model-item label="识别规则" prop="identifyRule">
				<a-input v-model="form.identifyRule" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="拆分规则" prop="splitRule">
				<a-input v-model="form.splitRule" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="截断规则" prop="truncationRule">
				<a-input v-model="form.truncationRule" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="替换规则" prop="replaceRule">
				<a-input v-model="form.replaceRule" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
	...
	data() {
		return {
			...
			rules: {
				...
				truncationRule: [
					{ required: true, message: '请输入截断规则', trigger: 'blur' },
				],
				templateId: [
					{ required: true, message: '请选择范本', trigger: 'change' },
				],
				replaceRule: [
					{ required: true, message: '请输入替换规则', trigger: 'blur' },
				],
			},
			modelOptions: []
		}
			...
			this.$refs.ruleForm.validate(valid => {
				...
				if (valid) {
					if (this.title == '新增') {
						...
						const data = JSON.parse(JSON.stringify(this.form))
						generalUpdateApi(data).then(res => {
							this.handleOkLoading = false
							const data = res.data
							if (data.code == 200) {
								this.$refs.ruleForm.resetFields()		// 清空表单

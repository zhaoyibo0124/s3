        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    title: '规则类型',
                    dataIndex: 'ruleType',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.regexType.filter(item => item.code == record.ruleType).map(item =>
                        item.name)}</span>,
                    width: 160,
                },
                {
                ...
                {
                    title: '无效规则',
                    dataIndex: 'invalidRule',
                    ellipsis: true,
                    width: 160,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.invalidRule}>{record.invalidRule}</a-tooltip>
                },
                {
                    title: '拆分规则',
                    dataIndex: 'splitRule',
                ...
                },
                {
                    title: '是否重复',
                    dataIndex: 'isRepeat',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.yesNoType.filter(item => item.code == record.isRepeat).map(item =>
                        item.name)}</span>,
                    width: 100,
                },
                {
        ...
        deleteBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    importantDeleteApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
        ...
        disableBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    importantDisableApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    importantRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

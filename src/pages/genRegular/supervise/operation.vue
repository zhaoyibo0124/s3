	...
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="项目分类" prop="projectClassify">
				<a-select v-model="form.projectClassify" placeholder="请选择" 
					:disabled="title == '查看'">
			...
			<a-form-model-item label="监督关键字" prop="superviseKey">
				<a-input placeholder="请输入" v-model="form.superviseKey" 
					:disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
	...
	data() {
		return {
			...
			rules: {
				...
				superviseKey: [
					{ required: true, message: '请输入监督关键字', trigger: 'blur' },
				],
			}
		}
	},
			...
			this.$refs.ruleForm.validate(valid => {
				...
				if (valid) {
					if (this.title == '新增') {
						...
						const data = JSON.parse(JSON.stringify(this.form))
						superviseUpdateApi(data).then(res => {
							this.handleOkLoading = false
							const data = res.data
							if (data.code == 200) {
								this.$refs.ruleForm.resetFields()		// 清空表单

<template>
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="媒介关键字" prop="mediumKey">
				<a-input v-model="form.mediumKey" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
...
export default {
	...
	data() {
		return {
			...
			rules: {
				mediumKey: [
					{ required: true, message: '请输入媒介关键字', trigger: 'blur' },
				],
			}
		}
	},

<template>
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="关键字名称" prop="keyword">
				<a-input v-model="form.keyword" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
			...
			this.$refs.ruleForm.validate(valid => {
				...
				if (valid) {
					if (this.title == '新增') {
						...
						const data = JSON.parse(JSON.stringify(this.form))
						keywordUpdateApi(data).then(res => {
							this.handleOkLoading = false
							const data = res.data
							if (data.code == 200) {
								this.$refs.ruleForm.resetFields()		// 清空表单

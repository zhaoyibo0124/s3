<template>
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="区域关键字" prop="regionKey">
				<a-input style="width: 100%" placeholder="请输入" v-model="form.regionKey" :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="本级排序" prop="levelSort">
				<a-input-number style="width: 100%" :min="0" :max="9999" v-model="form.levelSort" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="预算金额" prop="budget">
				<a-input placeholder="请输入" v-model="form.budget" style="width: 100%" :disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
	...
	data() {
		return {
			...
			rules: {
				...
				levelSort: [
					{ required: true, message: '请输入本级排序', trigger: 'change' },
				],
				budget: [
			...
			this.$refs.ruleForm.validate(valid => {
				...
				if (valid) {
					if (this.title == '新增') {
						...
						delete data.id
						regionBudgetAddApi(data).then(res => {
						...
						const data = JSON.parse(JSON.stringify(this.form))
						regionBudgetUpdateApi(data).then(res => {
							this.handleOkLoading = false
							const data = res.data
							if (data.code == 200) {
								this.$refs.ruleForm.resetFields()		// 清空表单

<template>
	<div class="bidSubregion" :style="`min-height: ${pageMinHeight}px`">
...
import {
	regionBudgetDeleteApi,
	regionBudgetDisableApi,
	regionBudgetRecoveryApi,
	regionBudgetQueryListApi,
...
export default {
	...
	methods: {
		...
		queryListData(page, pageSize) {
			...
			regionBudgetQueryListApi(data).then(res => {
				// 全局loading隐藏
		...
		deleteBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					regionBudgetDeleteApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
		...
		disableBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					regionBudgetDisableApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
		...
		restoreBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					regionBudgetRecoveryApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

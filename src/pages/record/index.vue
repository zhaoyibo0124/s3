<template>
    <div class="bidSubproject" :style="`min-height: ${pageMinHeight}px`">
            ...
            <div class="content-box">
                <ul class="my-table">
                    <li v-for="record in dataSource" :key="record.id">
                        <div class="thumbnail">
                            <div
                                :class="['status', record.status == 0 ? 'gray' : record.status == 1 ? 'blue' : record.status == 2 ? 'green' : 'red']">
                                {{ colors[record.status] }}
                            </div>
                            <!-- <img class="icon" src="../../assets/img/icon4.png" alt=""> -->
                        </div>
                        <div class="info">
                            <div class="name" @click="viewBtn(record)">{{ record.archivesName }}</div>
                            <!-- <div class="text">{{ record.archivesName }}</div> -->
                            <div class="code-info">
                                <div class="code">{{ record.archivesCode }}</div>
                                <div class="type" v-if="record.type">{{ record.type }}</div>
                            </div>
                            <div class="desc">
        ...
        <addModal v-if="addFlag" :title="title" :defaultData="defaultData" @confirmValue="confirmAdd"
            @cancelValue="cancelAdd"></addModal>
    </div>
</template>

<script>
import addModal from './operation'
            ...
            dataSource: [
                {
                    ...
                    archivesName: '标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名标段名',

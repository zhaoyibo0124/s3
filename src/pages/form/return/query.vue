				...
				>
					...
					<template slot="yesNoType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.yesNoType" :key="index">
							<span v-if="item.code == record.isConvert">{{item.name}}</span>
						</div>
					</template>
					<template slot="returnType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.returnType" :key="index">
							<span v-if="item.code == record.returnType">{{item.name}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/form/return/operation'
	import {
	...
	const columns = [
		...
		{
			title: '返回注释',
			dataIndex: 'returnNotes',
		},
		{
			title: '返回排序',
			dataIndex: 'returnSort',
		},
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '16fcde00f8b14bf39d163c9723cfdfb5'
				}
				...
				CustomizeReturnQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.returnName = item.returnName
							template.returnType = item.returnType
							template.isConvert = item.isConvert
							template.dataSetType = item.dataSetType
							template.dataSet = item.dataSet
							template.returnNotes = item.returnNotes
							template.returnSort = item.returnSort
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeReturnDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeReturnDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeReturnRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

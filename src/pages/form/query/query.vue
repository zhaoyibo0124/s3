			...
			<a-card v-if="operation.table">
				...
				>
					...
					<template slot="queryType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.queryType" :key="index">
							<span v-if="item.code == record.queryType">{{item.name}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/form/query/operation'
	import {
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'cfde0d1d50f6414a99d1aa9e45b05d7e'
				}
				...
				CustomizeQueryQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.queryName = item.queryName
							template.queryType = item.queryType
							template.requiredFields = item.requiredFields
							template.elementType = item.elementType
							template.dataSetType = item.dataSetType
							template.dataSet = item.dataSet
							template.queryNotes = item.queryNotes
							template.querySort = item.querySort
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeQueryDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeQueryDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeQueryRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

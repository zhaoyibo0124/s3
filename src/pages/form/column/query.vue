		...
		<div class="gutter-box">
			...
			<a-card v-if="operation.table">
				...
				<a-table
					...
					:scroll="{x: 1200}"
				>
					<template slot="formIdName" slot-scope="text, record">
						<div v-for="(item,index) in formList" :key="index">
							<span v-if="item.value == record.formId">{{item.key}}</span>
						</div>
					</template>
					<template slot="columnType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.columnType" :key="index">
							<span v-if="item.code == record.columnType">{{item.name}}</span>
						</div>
					</template>
					<template slot="yesNoType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.yesNoType" :key="index">
							<span v-if="item.code == record.isNull">{{item.name}}</span>
						</div>
					</template>
					<template slot="elementType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.elementType" :key="index">
							<span v-if="item.code == record.elementType">{{item.name}}</span>
						</div>
					</template>
					<template slot="dataSetType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.dataSetType" :key="index">
							<span v-if="item.code == record.dataSetType">{{item.name}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/form/column/operation'
	import {
	...
	const columns = [
		...
		},
		{
			title: '表单名称',
			dataIndex: 'formId',
			scopedSlots: {customRender: 'formIdName'},
		},
		{
			title: '字段名称',
		...
		},
		{
			...
			scopedSlots: {customRender: 'dataSetType'},
		},
		{
			title: '数据集',
			dataIndex: 'dataSet',
	...
	export default {
		...
		created: function(){
			this.searchForm.formId = this.$route.query.id
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'a39c206ae973411bbd27f2b5ce86054c'
				}
				...
				CustomizeColumnQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.formId = item.formId
							template.columnName = item.columnName
							template.columnType = item.columnType
							template.columnLength = item.columnLength
							template.isNull = item.isNull
							template.elementType = item.elementType
							template.dataSetType = item.dataSetType
							template.dataSet = item.dataSet
							template.columnNotes = item.columnNotes
							template.columnSort = item.columnSort
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeColumnDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeColumnDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						CustomizeColumnRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

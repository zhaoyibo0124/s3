...
import {
    contradictionDeleteApi,
    contradictionDisableApi,
    contradictionRecoveryApi,
    contradictionQueryListApi,
    PowerFindPowerOperation,
    modelListApi
} from '@/services/commentApiList'
export default {
    name: 'contradictionPage',
    components: {
        infoAlert
    },
    data() {
        ...
        return {
            ...
            columns: [
                ...
                {
                    title: '矛盾名称',
                    dataIndex: 'contradictionName',
                    width: 180
                },
                {
                    title: '识别规则',
                    dataIndex: 'identifyRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.identifyRule}>{record.identifyRule}</a-tooltip>
                },
                {
                    title: '内容规则',
                    dataIndex: 'contentRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.contentRule}>{record.contentRule}</a-tooltip>
                },
                {
                    title: '替换规则',
                    dataIndex: 'replaceRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.replaceRule}>{record.replaceRule}</a-tooltip>
                },
                {
                    title: '拆分规则',
                    dataIndex: 'splitRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.splitRule}>{record.splitRule}</a-tooltip>
                },
                {
                    title: '开始规则',
                    dataIndex: 'startRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.startRule}>{record.startRule}</a-tooltip>
                },
                {
                    title: '向上规则',
                    dataIndex: 'upRule',
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.upRule}>{record.upRule}</a-tooltip>
                },
                {
                    title: '向下取几行',
                    dataIndex: 'downLine',
                ...
                },
                {
                    title: '是否引用标题',
                    dataIndex: 'isTitle',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.yesNoType.filter(item => item.code == record.isTitle).map(item =>
                        item.name)}</span>,
                    width: 180
                },
                {
    ...
    methods: {
        ...
        queryListData(page, pageSize) {
            ...
            contradictionQueryListApi(data).then(res => {
                // 全局loading隐藏
        ...
        deleteBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    contradictionDeleteApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
        ...
        disableBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    contradictionDisableApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    contradictionRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

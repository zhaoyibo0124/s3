...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    title: '金额规则',
                    dataIndex: 'moneyRule',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.comparisonType.filter(item => item.code == record.moneyRule).map(item =>
                        item.name)}</span>
                },
                {
                    title: '金额类型',
                    dataIndex: 'moneyType',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.lineType.filter(item => item.code == record.moneyType).map(item =>
                        item.name)}</span>
                },
                {
                    title: '金额值',
                ...
                },
                {
                    ...
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <span>{this.contentOptions.filter(item => item.id == record.fileMoney).map(item =>
                ...
                },
                {
                    ...
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <span>{this.contentOptions.filter(item => item.id == record.fileWay).map(item =>
                        item.contentName)}</span>,
                },
                {
                    title: '操作',
    ...
    methods: {
        ...
        queryListData(page, pageSize) {
            ...
            moneyQueryListApi(data).then(res => {
                // 全局loading隐藏
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    moneyRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

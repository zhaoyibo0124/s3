<template>
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="金额名称" prop="moneyName">
				<a-input placeholder="请输入" v-model="form.moneyName"  :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="金额规则" prop="moneyRule">
				<a-select v-model="form.moneyRule" placeholder="请选择"  :disabled="title == '查看'">
					<a-select-option v-for="item, index in queryJsonBasicList.comparisonType" :key="index"
						:value="item.code">{{
							item.name }}</a-select-option>
				</a-select>
			</a-form-model-item>
			<a-form-model-item label="金额类型" prop="moneyType">
				<a-select v-model="form.moneyType" placeholder="请选择"  :disabled="title == '查看'">
					<a-select-option v-for="item, index in queryJsonBasicList.lineType" :key="index"
						:value="item.code">{{
							item.name }}</a-select-option>
				</a-select>
			</a-form-model-item>
			<a-form-model-item label="金额值" prop="moneyValue">
				<a-input placeholder="请输入" v-model="form.moneyValue"  :disabled="title == '查看'" />
			</a-form-model-item>
			<a-form-model-item label="文件发售金额" prop="fileMoney">
				<a-select v-model="form.fileMoney" placeholder="请选择"  :disabled="title == '查看'">
					<a-select-option v-for="item, index in contentOptions" :key="index" :value="item.id">{{
						item.contentName
						}}</a-select-option>
				</a-select>
			</a-form-model-item>
			<a-form-model-item label="文件发售方式" prop="fileWay">
				<a-select v-model="form.fileWay" placeholder="请选择"  :disabled="title == '查看'">
					<a-select-option v-for="item, index in contentOptions" :key="index" :value="item.id">{{
						item.contentName
						}}</a-select-option>
				</a-select>
	...
	data() {
		return {
			...
			rules: {
				...
				moneyName: [
					{ required: true, message: '请输入金额名称', trigger: 'blur' },
				],
				moneyRule: [
					{ required: true, message: '请选择金额规则', trigger: 'change' },
				],
				moneyType: [
					{ required: true, message: '请选择金额类型', trigger: 'change' },
				],
				moneyValue: [
					{ required: true, message: '请输入金额值', trigger: 'blur' },
				],
				fileMoney: [
					{ required: true, message: '请选择文件发售金额', trigger: 'change' },
				],
				fileWay: [
					{ required: true, message: '请选择文件发售方式', trigger: 'change' },
				],
			},
			modelOptions: [],
			contentOptions: []

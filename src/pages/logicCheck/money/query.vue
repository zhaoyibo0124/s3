...
import {
	moneyLogicDeleteApi,
	moneyLogicDisableApi,
	moneyLogicRecoveryApi,
	moneyLogicQueryListApi,
...
export default {
	...
	methods: {
		...
		queryListData(page, pageSize) {
			...
			moneyLogicQueryListApi(data).then(res => {
				// 全局loading隐藏
		...
		deleteBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					moneyLogicDeleteApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
		...
		disableBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					moneyLogicDisableApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
		...
		restoreBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					moneyLogicRecoveryApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

			...
			<a-card v-if="operation.table">
				...
				>
					<div slot="action" slot-scope="text, record">
						<a-space>
							<a-button type="link" v-if="record.isReset == 2 && record.operationGroup && record.operationResetSql" @click="recoveryBtn(record)">
								还 原
							</a-button>
							<a-button type="link" v-if="record.isReset == 1 && record.operationGroup && record.operationResetSql" @click="reductionBtn(record)">
								恢 复
							</a-button>
						</a-space>
					</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '90f21e32ba30496283c2c8f155a999f6'
				}
			...
			queryListData(page, pageSize) {
				...
				LogFormQueryList(data).then(res => {
					// 全局loading隐藏

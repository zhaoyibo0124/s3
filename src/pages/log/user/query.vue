			...
			<a-card v-if="operation.table">
				...
				>
					<div slot="action" slot-scope="text, record">
						<a-space>
							<a-button type="link" v-if="record.isReset == 2" @click="recoveryBtn(record)">
								还 原
							</a-button>
							<a-button type="link" v-if="record.isReset == 1" @click="reductionBtn(record)">
								恢 复
							</a-button>
						</a-space>
					</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '0627d6084f134296b6eaf24db54361d9'
				}
			...
			queryListData(page, pageSize) {
				...
				LogUserQueryList(data).then(res => {
					// 全局loading隐藏

...
<script>
	...
	import infoAlert from '@/pages/office/down/operation'
	import {
		DownFileDelete,
		DownFileDisable,
		DownFileRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '968549e826b943d4ae5eec4df816ba60'
				}
			...
			queryListData(page, pageSize) {
				...
				DownFileQueryList(data).then(res => {
					// 全局loading隐藏
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						DownFileRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

...
<script>
	...
	import infoAlert from '@/pages/office/change/operation'
	import {
		ChangeFileDelete,
		ChangeFileDisable,
		ChangeFileRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'ba32a38c4bb943c7997d0ee704fe7f98'
				}
			...
			queryListData(page, pageSize) {
				...
				ChangeFileQueryList(data).then(res => {
					// 全局loading隐藏
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						ChangeFileDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						ChangeFileDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						ChangeFileRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

...
<script>
	...
	import infoAlert from '@/pages/office/upload/operation'
	import {
		UploadFileDelete,
		UploadFileDisable,
		UploadFileRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'e699b2e96a0d4a75a4027585b6af4c0f'
				}
			...
			queryListData(page, pageSize) {
				...
				UploadFileQueryList(data).then(res => {
					// 全局loading隐藏
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						UploadFileDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						UploadFileDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						UploadFileRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

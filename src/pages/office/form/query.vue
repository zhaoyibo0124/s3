...
<script>
	...
	import infoAlert from '@/pages/office/form/operation'
	import {
		FormFileDelete,
		FormFileDisable,
		FormFileRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '9d5d02ae1b0d4abab99d580ef9c591a0'
				}
			...
			queryListData(page, pageSize) {
				...
				FormFileQueryList(data).then(res => {
					// 全局loading隐藏
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						FormFileRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

		...
		<div class="gutter-box">
				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
                        <a-form-model-item label="文件名称">
							<a-input placeholder="请输入" v-model="searchForm.fileAlias" style="width: 174px;" />
						</a-form-model-item>
			...
			<a-card v-if="operation.table">
				...
				<a-table
					...
					:scroll="{x: 1200}"
				>
					<template slot="fileClassifyType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.fileClassifyType" :key="index">
							<span v-if="item.code == record.fileClassify">{{item.name}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/office/remove/operation'
	import {
		RemoveFileDelete,
		RemoveFileDisable,
		RemoveFileRecovery,
	...
	const columns = [
		...
		{
			title: '文件分类',
			dataIndex: 'fileClassify',
			scopedSlots: {customRender: 'fileClassifyType'},
		},
		{
			title: '文件主键',
			dataIndex: 'fileParentId',
		},
		{
			title: '文件别名',
			dataIndex: 'fileAlias',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'e701b0527ab4436a82f459ec2820dafb'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
                    fileAlias: this.searchForm.fileAlias,
				...
				RemoveFileQueryList(data).then(res => {
					// 全局loading隐藏
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.fileClassify = item.fileClassify
							template.fileParentId = item.fileParentId
							template.fileName = item.fileName
							template.fileAlias = item.fileAlias
							template.fileType = item.fileType
							template.filePath = item.filePath
							template.fileUrl = item.fileUrl
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						RemoveFileDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						RemoveFileDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						RemoveFileRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

...
import {
	basicErrorDeleteApi,
	basicErrorDisableApi,
	basicErrorRecoveryApi,
	basicErrorQueryListApi,
...
export default {
	...
	methods: {
		...
		queryListData(page, pageSize) {
			...
			basicErrorQueryListApi(data).then(res => {
				// 全局loading隐藏
		...
		deleteBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					basicErrorDeleteApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
		...
		disableBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					basicErrorDisableApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
		...
		restoreBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					basicErrorRecoveryApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

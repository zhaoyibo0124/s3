<template>
	<a-modal :bodyStyle="{ padding: 0 }" :title="title" :visible="visible" @cancel="handleCancel"
		...
		<a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol"
			...
			<a-form-model-item label="进场关键字" prop="approachKey">
				<a-input v-model="form.approachKey" placeholder="请输入" :disabled="title == '查看'" />
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
...
export default {
	...
	data() {
		return {
			...
			rules: {
				approachKey: [
					{ required: true, message: '请输入进场关键字', trigger: 'blur' },
				],
			}
		}
	},
			...
			this.$refs.ruleForm.validate(valid => {
				...
				if (valid) {
					if (this.title == '新增') {
						...
						const data = JSON.parse(JSON.stringify(this.form))
						approachUpdateApi(data).then(res => {
							this.handleOkLoading = false
							const data = res.data
							if (data.code == 200) {
								this.$refs.ruleForm.resetFields()		// 清空表单

				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="所属企业">
							<a-select v-model="searchForm.enterpriseParent" placeholder="请选择" style="width: 174px;">
						...
						<a-form-model-item label="企业编码">
							<a-input placeholder="请输入" v-model="searchForm.enterpriseCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="企业名称">
							<a-input placeholder="请输入" v-model="searchForm.enterpriseName" style="width: 174px;" />
						</a-form-model-item>
				...
				>
					<template slot="secondLevelType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.secondLevelType" :key="index">
							<span v-if="item.code == record.enterpriseLevel">{{item.name}}</span>
						</div>
					</template>
					<template slot="enterpriseParentType" slot-scope="text, record">
						<div v-for="(item,index) in enterpriseList" :key="index">
							<span v-if="item.value == record.enterpriseParent">{{item.key}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/organization/enterprise/operation'
	import {
		EnterpriseDelete,
		EnterpriseDisable,
		EnterpriseRecovery,
	...
	const columns = [
		...
		{
			title: '企业上级',
			dataIndex: 'enterpriseParent',
			scopedSlots: {customRender: 'enterpriseParentType'},
		},
		{
			title: '企业编码',
		...
		{
			title: '企业排序',
			dataIndex: 'enterpriseSort',
		},
		{
			title: '企业备注',
			dataIndex: 'enterpriseContent',
		},
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'f99d488cc43849d18286ca0f4869ea8f'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
					enterpriseParent: this.searchForm.enterpriseParent,
				...
				EnterpriseQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.enterpriseCode = item.enterpriseCode
							template.enterpriseName = item.enterpriseName
							template.enterpriseSort = item.enterpriseSort
							template.enterpriseContent = item.enterpriseContent
							template.enterpriseLevel = item.enterpriseLevel
							template.enterpriseParent = item.enterpriseParent
							...
							template.enterpriseRoleIds = item.enterpriseRoleIds
							template.enterpriseSystemIds = item.enterpriseSystemIds
			...
			handleReset() {
				// 清空表单
				this.searchForm.enterpriseParent = undefined
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						EnterpriseDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						EnterpriseDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						EnterpriseRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="所属职位">
							<a-select v-model="searchForm.positionParent" placeholder="请选择" style="width: 174px;">
								<a-select-option v-for="(item,index) in positionList" :value="item.value" :key="index">
									{{item.key}}
								</a-select-option>
							</a-select>
						</a-form-model-item>
						<a-form-model-item label="职位编码">
							<a-input placeholder="请输入" v-model="searchForm.positionCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="职位名称">
							<a-input placeholder="请输入" v-model="searchForm.positionName" style="width: 174px;" />
						</a-form-model-item>
				...
				>
					<template slot="secondLevelType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.secondLevelType" :key="index">
							<span v-if="item.code == record.positionLevel">{{item.name}}</span>
						</div>
					</template>
					<template slot="positionParentType" slot-scope="text, record">
						<div v-for="(item,index) in positionList" :key="index">
							<span v-if="item.value == record.positionParent">{{item.key}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/organization/position/operation'
	import {
		PositionDelete,
		PositionDisable,
		PositionRecovery,
	...
	const columns = [
		...
		},
		{
			...
			scopedSlots: {customRender: 'positionParentType'},
		},
		{
			title: '职位编码',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '133edf0f869a484bbc61908acf49f1cb'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
					positionParent: this.searchForm.positionParent,
					positionCode: this.searchForm.positionCode,
				...
				PositionQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.positionCode = item.positionCode
							template.positionName = item.positionName
							template.positionSort = item.positionSort
							template.positionContent = item.positionContent
							template.positionLevel = item.positionLevel
							template.positionParent = item.positionParent
							...
							template.positionRoleIds = item.positionRoleIds
							template.positionSystemIds = item.positionSystemIds
							template.positionDepartmentIds = item.positionDepartmentIds
			...
			handleReset() {
				// 清空表单
				this.searchForm.positionParent = undefined
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						PositionRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

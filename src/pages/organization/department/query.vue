				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="部门编码">
							<a-input placeholder="请输入" v-model="searchForm.departmentCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="部门名称">
							<a-input placeholder="请输入" v-model="searchForm.departmentName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/organization/department/operation'
	import {
		DepartmentDelete,
		DepartmentDisable,
		DepartmentRecovery,
	...
	const columns = [
		...
		{
			title: '部门排序',
			dataIndex: 'departmentSort',
		},
		{
			title: '部门备注',
			dataIndex: 'departmentContent',
		},
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '15049f8cc15b4025884e67f53936dcb9'
				}
				...
				DepartmentQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.departmentCode = item.departmentCode
							template.departmentName = item.departmentName
							template.departmentSort = item.departmentSort
							template.departmentContent = item.departmentContent
							...
							template.departmentRoleIds = item.departmentRoleIds
							template.departmentSystemIds = item.departmentSystemIds
							template.departmentEnterpriseIds = item.departmentEnterpriseIds
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						DepartmentRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.table){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

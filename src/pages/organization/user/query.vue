				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="用户名称">
							<a-input placeholder="请输入" v-model="searchForm.userName" style="width: 174px;" />
						</a-form-model-item>
						<a-form-model-item label="用户手机号">
							<a-input placeholder="请输入" v-model="searchForm.userMobile" style="width: 174px;" />
						</a-form-model-item>
						<a-form-model-item label="用户身份证号">
							<a-input placeholder="请输入" v-model="searchForm.userCard" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="用户账号">
							<a-input placeholder="请输入" v-model="searchForm.loginName" style="width: 174px;" />
						</a-form-model-item>
				...
				>
					<div slot="action" slot-scope="text, record">
						<a-space>
							...
							<a-button type="link" @click="resetBtn(record)" v-if="operation.update">
								重置密码
							</a-button>
						</a-space>
					</div>
...
<script>
	...
	import infoAlert from '@/pages/organization/user/operation'
	import {
		UserDelete,
		UserDisable,
		UserRecovery,
	...
	const columns = [
		...
		{
			title: '用户密码',
			dataIndex: 'loginPasswordPlainText',
		},
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '61951bdeb6fb41f2b3fe20ff14be31fe'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
					...
					userCard: this.searchForm.userCard,
				...
				UserQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.userName = item.userName
							template.userMobile = item.userMobile
							template.userCard = item.userCard
							template.loginName = item.loginName
							template.loginPasswordPlainText = item.loginPasswordPlainText
							template.userVersion = item.userVersion
							...
							template.userGroupIds = item.userGroupIds
							template.userRoleIds = item.userRoleIds
							template.enterpriseId = item.userOrganizationIds[0]
							template.departmentId = item.userOrganizationIds[1]
							template.positionId = item.userOrganizationIds[2]
							template.userSystemIds = item.userSystemIds
			...
			resetBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						UserResetPassword(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

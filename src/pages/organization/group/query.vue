			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="用户组编码">
							<a-input placeholder="请输入" v-model="searchForm.groupCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="用户组名称">
							<a-input placeholder="请输入" v-model="searchForm.groupName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/organization/group/operation'
	import {
		GroupDelete,
		GroupDisable,
		GroupRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '2df52e01d0b2477f934eae25a0d22b40'
				}
				...
				GroupQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.groupCode = item.groupCode
							template.groupName = item.groupName
							template.groupContent = item.groupContent
							template.groupSort = item.groupSort
							...
							template.groupRoleIds = item.groupRoleIds
							template.groupSystemIds = item.groupSystemIds

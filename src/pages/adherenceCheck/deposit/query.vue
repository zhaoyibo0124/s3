...
import {
	depositComplianceDeleteApi,
	depositComplianceDisableApi,
	depositComplianceRecoveryApi,
	depositComplianceQueryListApi,
...
export default {
	...
	methods: {
		...
		queryListData(page, pageSize) {
			...
			depositComplianceQueryListApi(data).then(res => {
				// 全局loading隐藏
		...
		deleteBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					depositComplianceDeleteApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
		...
		disableBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					depositComplianceDisableApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
		...
		restoreBtn(record) {
			...
			this.$confirm({
				...
				onOk() {
					...
					depositComplianceRecoveryApi(data).then(res => {
						const data = res.data
						if (data.code == 200) {
							if (that.operation.queryList) {
								that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

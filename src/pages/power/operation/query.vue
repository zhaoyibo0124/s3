			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="功能编码">
							<a-input placeholder="请输入" v-model="searchForm.operationCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="功能名称">
							<a-input placeholder="请输入" v-model="searchForm.operationName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/operation/operation'
	import {
		OperationDelete,
		OperationDisable,
		OperationRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '4d5129587ce244f0926ff6ecb3e0efb6'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
                    operationCode: this.searchForm.operationCode,
                    operationName: this.searchForm.operationName,
				...
				OperationQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.operationCode = item.operationCode
							template.operationName = item.operationName
							template.operationContent = item.operationContent
							template.operationSort = item.operationSort
			...
			handleReset() {
				// 清空表单
                this.searchForm.operationCode = ''
                this.searchForm.operationName = ''
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						OperationDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						OperationRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

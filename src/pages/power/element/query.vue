			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="元素编码">
							<a-input placeholder="请输入" v-model="searchForm.elementCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="元素名称">
							<a-input placeholder="请输入" v-model="searchForm.elementName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/element/operation'
	import {
		ElementDelete,
		ElementDisable,
		ElementRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '7460b19750a44daf838244a618493398'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
                    elementCode: this.searchForm.elementCode,
				...
				ElementQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.elementCode = item.elementCode
							template.elementName = item.elementName
							template.elementContent = item.elementContent
							template.elementSort = item.elementSort
			...
			handleReset() {
				// 清空表单
                this.searchForm.elementCode = ''
                this.searchForm.elementName = ''

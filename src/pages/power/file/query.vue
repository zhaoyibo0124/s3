				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
                        <a-form-model-item label="文件名称">
							<a-input placeholder="请输入" v-model="searchForm.fileName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/file/operation'
	import {
		FileDelete,
		FileDisable,
		FileRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'cc2ff4d4607c4475b151edd8eddc5d10'
				}
				...
				FileQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.fileContent = item.fileContent
							template.fileSort = item.fileSort

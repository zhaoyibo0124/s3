				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="系统编码">
							<a-input placeholder="请输入" v-model="searchForm.systemCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="系统名称">
							<a-input placeholder="请输入" v-model="searchForm.systemName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/system/operation'
	import {
		SystemDelete,
		SystemDisable,
		SystemRecovery,
	...
	const columns = [
        ...
        {
			title: '系统排序',
			dataIndex: 'systemSort',
		},
		{
			title: '系统备住',
			dataIndex: 'systemContent',
		},
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			queryListData(page, pageSize) {
				...
				const data = {
                    systemCode: this.searchForm.systemCode,
				...
				SystemQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.systemCode = item.systemCode
							template.systemName = item.systemName
							template.systemContent = item.systemContent
							template.systemSort = item.systemSort

			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="权限编码">
							<a-input placeholder="请输入" v-model="searchForm.powerCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="权限名称">
							<a-input placeholder="请输入" v-model="searchForm.powerName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/power/operation'
	import {
		PowerDelete,
		PowerDisable,
		PowerRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'e9c55e4ac51648f1b555c6216180ddbe'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
                    powerCode: this.searchForm.powerCode,
				...
				PowerQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.powerCode = item.powerCode
							template.powerName = item.powerName
							template.powerContent = item.powerContent
							template.powerSort = item.powerSort
							...
							template.powerSystemIds = item.powerSystemIds
							template.powerList = item.powerList

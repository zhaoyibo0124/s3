				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="角色编码">
							<a-input placeholder="请输入" v-model="searchForm.roleCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="角色名称">
							<a-input placeholder="请输入" v-model="searchForm.roleName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/power/role/operation'
	import {
		RoleDelete,
		RoleDisable,
		RoleRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'e38d309020c34bb0b36cc82bea19dac9'
				}
				...
				RoleQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.id = item.id
							template.roleCode = item.roleCode
							template.roleName = item.roleName
							template.roleContent = item.roleContent
							template.roleSort = item.roleSort
							...
							template.roleSystemIds = item.roleSystemIds
							template.rolePowerIds = item.rolePowerIds

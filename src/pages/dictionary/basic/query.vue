				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="所属字典">
							<a-select v-model="searchForm.basicParent" placeholder="请选择" style="width: 174px;">
						...
						<a-form-model-item label="字典编码">
							<a-input placeholder="请输入" v-model="searchForm.basicCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="字典名称">
							<a-input placeholder="请输入" v-model="searchForm.basicName" style="width: 174px;" />
						</a-form-model-item>
				...
				>
					<template slot="secondLevelType" slot-scope="text, record">
						<div v-for="(item,index) in queryJsonBasicList.secondLevelType" :key="index">
							<span v-if="item.code == record.basicLevel">{{item.name}}</span>
						</div>
					</template>
					<template slot="basicParentType" slot-scope="text, record">
						<div v-for="(item,index) in basicList" :key="index">
							<span v-if="item.value == record.basicParent">{{item.key}}</span>
						</div>
					</template>
...
<script>
	...
	import infoAlert from '@/pages/dictionary/basic/operation'
	import {
		BasicDelete,
		BasicDisable,
		BasicRecovery,
	...
	const columns = [
		...
		{
			title: '字典级别',
			dataIndex: 'basicLevel',
			scopedSlots: {customRender: 'secondLevelType'},
		},
		{
			title: '字典上级',
			dataIndex: 'basicParent',
			scopedSlots: {customRender: 'basicParentType'},
		},
		{
			title: '字典编码',
			dataIndex: 'basicCode',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '3bb2101367df4f23839eb1cba193b312'
				}
			...
			queryListData(page, pageSize) {
				...
				const data = {
					basicParent: this.searchForm.basicParent,
                    basicCode: this.searchForm.basicCode,
                    basicName: this.searchForm.basicName,
				...
				BasicQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.basicParent = item.basicParent
							template.basicCode = item.basicCode
							template.basicName = item.basicName
							template.basicLevel = item.basicLevel
							template.basicSort = item.basicSort
			...
			handleReset() {
				// 清空表单
				this.searchForm.basicParent = undefined
                this.searchForm.basicCode = ''
                this.searchForm.basicName = ''

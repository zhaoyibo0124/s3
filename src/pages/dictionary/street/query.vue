				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="所属省份">
							<a-select v-model="searchForm.provinceCode" @change="queryCityList(searchForm.provinceCode)" placeholder="请选择" style="width: 174px;">
						...
						<a-form-model-item label="所属市区">
							<a-select v-model="searchForm.cityCode" @change="queryAreaList(searchForm.cityCode)" placeholder="请选择" style="width: 174px;">
						...
						<a-form-model-item label="所属地区">
							<a-select v-model="searchForm.areaCode" placeholder="请选择" style="width: 174px;">
								<a-select-option v-for="(item,index) in areaList" :value="item.value" :key="index">
									{{item.key}}
								</a-select-option>
							</a-select>
						</a-form-model-item>
						<a-form-model-item label="街道编码">
							<a-input placeholder="请输入" v-model="searchForm.streetCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="街道名称">
							<a-input placeholder="请输入" v-model="searchForm.streetName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/dictionary/street/operation'
	import {
		StreetDelete,
		StreetDisable,
		StreetRecovery,
	...
	const columns = [
        ...
        {
			title: '街道排序',
			dataIndex: 'streetSort',
		},
		{
			title: '街道简称',
			dataIndex: 'streetAlias',
		},
		{
			title: '街道经度',
			dataIndex: 'streetLnt',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'c06bd011e9c14d0082d6469f1ee4a901'
				}
				...
				StreetQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.areaCode = item.areaCode
							template.streetCode = item.streetCode
							template.streetName = item.streetName
							template.streetAlias = item.streetAlias
							template.streetLnt = item.streetLnt
							template.streetLat = item.streetLat
							template.streetSort = item.streetSort
			...
			handleReset() {
				// 清空表单
				this.searchForm.provinceCode = undefined
				this.searchForm.cityCode = undefined

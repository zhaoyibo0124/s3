			...
			<div class="serchInfo">
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="省份编码">
							<a-input placeholder="请输入" v-model="searchForm.provinceCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="省份名称">
							<a-input placeholder="请输入" v-model="searchForm.provinceName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/dictionary/province/operation'
	import {
		ProvinceDelete,
		ProvinceDisable,
		ProvinceRecovery,
	...
	const columns = [
		...
		{
			title: '省份排序',
			dataIndex: 'provinceSort',
		},
		{
			title: '省份简称',
			dataIndex: 'provinceAlias',
		},
		{
			title: '省份经度',
			dataIndex: 'provinceLnt',
        },
        {
			title: '省份纬度',
			dataIndex: 'provinceLat',
        },
		{
			title: '操作',
			dataIndex: 'action',
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '6eafc5e0037f481d967346bedd03509c'
				}
				...
				ProvinceQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.provinceCode = item.provinceCode
							template.provinceName = item.provinceName
							template.provinceAlias = item.provinceAlias
							template.provinceLnt = item.provinceLnt
							template.provinceLat = item.provinceLat
							template.provinceSort = item.provinceSort
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						ProvinceRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

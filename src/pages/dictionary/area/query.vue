				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						...
						<a-form-model-item label="所属市区">
							<a-select v-model="searchForm.cityCode" placeholder="请选择" style="width: 174px;">
								<a-select-option v-for="(item,index) in cityList" :value="item.value" :key="index">
									{{item.key}}
								</a-select-option>
							</a-select>
						</a-form-model-item>
						<a-form-model-item label="地区编码">
							<a-input placeholder="请输入" v-model="searchForm.areaCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="地区名称">
							<a-input placeholder="请输入" v-model="searchForm.areaName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/dictionary/area/operation'
	import {
		AreaDelete,
		AreaDisable,
		AreaRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: 'd617f4be00714c4fa98f0b405847f72d'
				}
				...
				AreaQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
                            ...
                            template.id = item.id
							template.cityCode = item.cityCode
							template.areaCode = item.areaCode
							template.areaName = item.areaName
							template.areaAlias = item.areaAlias
							template.areaLnt = item.areaLnt
							template.areaLat = item.areaLat
							template.areaSort = item.areaSort

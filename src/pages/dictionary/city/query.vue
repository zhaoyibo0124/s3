				...
				<div :class="advanced ? 'search' : null" v-if="operation.find">
					<a-form-model layout="inline" ref="ruleForm" :model="searchForm">
						<a-form-model-item label="所属省份">
							<a-select v-model="searchForm.provinceCode" placeholder="请选择" style="width: 174px;">
								<a-select-option v-for="(item,index) in provinceList" :value="item.value" :key="index">
									{{item.key}}
								</a-select-option>
							</a-select>
						</a-form-model-item>
						<a-form-model-item label="市区编码">
							<a-input placeholder="请输入" v-model="searchForm.cityCode" style="width: 174px;" />
						</a-form-model-item>
                        <a-form-model-item label="市区名称">
							<a-input placeholder="请输入" v-model="searchForm.cityName" style="width: 174px;" />
						</a-form-model-item>
...
<script>
	...
	import infoAlert from '@/pages/dictionary/city/operation'
	import {
		CityDelete,
		CityDisable,
		CityRecovery,
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '322947fe0d1a47a18e2fe867a40304ef'
				}
				...
				CityQueryList(data).then(res => {
					...
					if(data.code == 200){
						...
						data.data.list.forEach((item, index) => {
							...
							template.cityName = item.cityName
							template.cityAlias = item.cityAlias
							template.cityLnt = item.cityLnt
							template.cityLat = item.cityLat
							template.citySort = item.citySort

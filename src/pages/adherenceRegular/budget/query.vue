...
import {
    budgetCalculateDeleteApi,
    budgetCalculateDisableApi,
    budgetCalculateRecoveryApi,
    budgetCalculateQueryListApi,
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    dataIndex: 'contentId',
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={
                        this.contentOptions.filter(item => item.id == record.contentId).map(item =>
                            item.contentName)
                    }>{this.contentOptions.filter(item => item.id == record.contentId).map(item =>
    ...
    methods: {
        ...
        queryListData(page, pageSize) {
            ...
            budgetCalculateQueryListApi(data).then(res => {
                // 全局loading隐藏
        ...
        deleteBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    budgetCalculateDeleteApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
        ...
        disableBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    budgetCalculateDisableApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    budgetCalculateRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

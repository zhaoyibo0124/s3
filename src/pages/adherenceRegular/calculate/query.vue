...
import {
    timeCalculateDeleteApi,
    timeCalculateDisableApi,
    timeCalculateRecoveryApi,
    timeCalculateQueryListApi,
    modelListApi,
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    ellipsis: true,
                    width: 180,
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={record.countName}>{record.countName}</a-tooltip>
                },
                {
                    title: '计算前内容',
                    dataIndex: 'contentIdFront',
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={
                        this.contentOptions.filter(item => item.id == record.contentIdFront).map(item =>
                            item.contentName)
                    }>{this.contentOptions.filter(item => item.id == record.contentIdFront).map(item =>
                        item.contentName)}</a-tooltip>,
                    ellipsis: true,
                    width: 300,
                },
                {
                    title: '计算后内容',
                    dataIndex: 'contentIdAfter',
                    customRender: (text, record) => <a-tooltip placement="topLeft" title={
                        this.contentOptions.filter(item => item.id == record.contentIdAfter).map(item =>
                            item.contentName)
                    }>{this.contentOptions.filter(item => item.id == record.contentIdAfter).map(item =>
                        item.contentName)}</a-tooltip>,
                    ellipsis: true,
                ...
                },
                {
                    ...
                    ellipsis: true,
                    width: 160,
                    customRender: (text, record) => <span>{this.queryJsonBasicList.comparisonType.filter(item => item.code == record.countRule).map(item =>
                        item.name)}</span>,
                },
                {
                    title: '计算值',
                ...
                },
                {
                    title: '是否工作日',
                    dataIndex: 'isWeekday',
                    customRender: (text, record) => <span>{this.queryJsonBasicList.yesNoType.filter(item => item.code == record.isWeekday).map(item =>
                        item.name)}</span>,
                    width: 120,
                },
                {
    ...
    methods: {
        ...
        queryListData(page, pageSize) {
            ...
            timeCalculateQueryListApi(data).then(res => {
                // 全局loading隐藏
        ...
        deleteBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    timeCalculateDeleteApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
        ...
        disableBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    timeCalculateDisableApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
        ...
        restoreBtn(record) {
            ...
            this.$confirm({
                ...
                onOk() {
                    ...
                    timeCalculateRecoveryApi(data).then(res => {
                        const data = res.data
                        if (data.code == 200) {
                            if (that.operation.queryList) {
                                that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

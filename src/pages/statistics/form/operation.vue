	...
	>
		<template slot="footer">
			<template v-if="title == '新增' || title == '编辑'">
				<a-button @click="verification">验证</a-button>
		...
		>
			...
			<a-form-model-item label="表单SQL" prop="formSql">
				<a-textarea rows="8" v-model="form.formSql" placeholder="请输入" :disabled="title == '查看'"/>
			</a-form-model-item>
			<a-form-model-item label="SQL执行结果">
				<a-textarea rows="8" v-model="form.formContent" :disabled="title == '查看'"/>
			</a-form-model-item>
		</a-form-model>
	</a-modal>
</template>
<script>
  ...
  export default {
    ...
    data() {
      return {
				...
				form: {
					...
					formSql: this.defaultData.data.formSql?this.defaultData.data.formSql:'',
					formContent: '',
				},
				rules: {
					formName: [
					...
					formSql: [
						{required: true, message: '请输入表单SQL', trigger: 'blur'},
					]
				}
			}
    },
    ...
    methods: {
			verification(){
			const data = {
				sql: this.form.formSql
			}
			StatisticsFormSql(data).then(res => {
				const data = res.data
				if (data.code == 200) {
				this.form.formContent = ''
				this.form.formContent = JSON.stringify(data.data)
				} else {
					...
					if(valid){
						if(this.title == '新增'){
							const data = {
								...
								formSql: this.form.formSql,
							}
							StatisticsFormAdd(data).then(res => {
							...
							StatisticsFormUpdate(data).then(res => {
								this.handleOkLoading = false
								const data = res.data
								if(data.code == 200){
									this.$refs.ruleForm.resetFields()		// 清空表单

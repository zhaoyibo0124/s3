...
<script>
	...
	import infoAlert from '@/pages/statistics/form/operation'
	import {
	...
	export default {
		...
		methods: {
			...
			findPowerOperation(){
				const data = {
					id: '47771459c20b430eb9e4a17fdc8b25c6'
				}
			...
			deleteBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						StatisticsFormDelete(data).then(res => {
							const data = res.data
							if(data.code == 200){
			...
			disableBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						StatisticsFormDisable(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)
			...
			restoreBtn(record) {
				...
				this.$confirm({
					...
					onOk() {
						...
						StatisticsFormRecovery(data).then(res => {
							const data = res.data
							if(data.code == 200){
								if(that.operation.queryList){
									that.queryListData(that.tablePagination.defaultCurrent, that.tablePagination.defaultPageSize)

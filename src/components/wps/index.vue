    ...
    <div class="mainContainer">
        <div class="urlbox">
            <div class="item input">
                <a-input v-model="MyOpenDoc" placeholder="这里演示切换文档" size="small" />
            </div>
            <div class="item" style="margin-left: 10px;">
                <a-button type="primary" @click="changeOpen()" size="small">切换打开</a-button>
            </div>
            <div class="item" style="margin-left: 10px;">
                <a-button size="small" @click="ReLoadFirst()" type="primary">完整嵌入</a-button>
            </div>
        </div>
        <div class="video-container" ref="player"></div>
    </div>
</template>

<script>
import GetDefaultConn from "../../assets/js/base"
//加载websoket类
...
export default {
    ...
    data() {
        return {
            ...
            ServerOpenFile: 'http://www.yuanmaster.com/Files/template.doc',
    ...
    methods: {
        ...
        openWebsocket(port, type) {
            ...
            socketClient.onError = () => {                // 避免IE中点击重复播放及firefox断开连接提示等问题
                if (!this.ReStartLoad && this.isConnService && !this.isDisConnect) {
                    //连接不上，认为还没有安装猿大师办公助手—文档在线编辑组件 没有安装时提示安装
                    this.$confirm('猿大师办公助手—文档在线编辑组件 服务端口连接失败，可能是尚未安装，是否马上下载安装？', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
        ...
        InsertContent() {
            ...
            let msg = {
                ...
                "rid": this.rid,
                "para": [{ "Paragraph": 1 },
                { "FontSize": "16", "FontName": "宋体", "Color": "0", "Bold": "8", "Text": "详情请访问猿大师办公助手网站：" },
                { "Paragraph": 1 },
                { "Address": "http://www.yuanofficer.com", "Display": "猿大师办公助手官方网站" }]
            }
            if (this.aid2 > 0 && this.aid2 == this.curID)
                this.socket[3].sendObj(msg)
            else
        ...
        SaveFile() {
            ...
            let msg = {
                "req": "Office_Save",
                "rid": this.rid,
                "para": { "Asyn": 0, "Agent": "", "Cookie": "", "Auth": "", "Para": "", "Url": `${BASE_URL}/examine/examine/exaTemplatePreparation/updateTemplateFile?id=${this.detailData.id}` }
            }
            if (this.aid2 > 0 && this.aid2 == this.curID) {
                this.socket[3].sendObj(msg)
            } else {
        ...
        DocRed() {
            ...
            let msg = {
                ...
                "para": {
                    ...
                    "File": "http://www.yuanmaster.com/Files/template.doc" // 模版文件，也可以是本地路径
                }
            }
            if (this.aid2 > 0 && this.aid2 == this.curID)
                this.socket[3].sendObj(msg)
        ...
        SendUpdateJson() {
            ...
            let msg = {
                ...
                "para": {
                    ...
                    "Date": "2025-02-19",
                    "Desc": "1、中间件高级版增加支持在夸克、遨游、猎豹及双核浏览器中的使用； 2、增强识别当前网页窗口信息接口，增强内嵌小程序启动参数可避免其加载到其它网页；3、优化高级版小程序在服务器版系统中的运行体验；4、Office网页组件增加其控件或VBA接口的全面调用方式，不再局限于单独封装的接口；5、猿大师办公助手网页组件增加OFD文档打开支持，解决在Win7等低版本系统直接关闭浏览器时可能弹崩溃问题，解决启动多个实例时设置的编辑权限不一样可能相互影响问题...",
                    "DownAddr": "http://local.zorrosoft.com/Files/Update/Office_Update.pid",
                    "Open": "http://test.yuanmaster.com/office",
